...
description: Use the local Grok Search Go CLI for current web search, URL fetch, site mapping, source-grounded answers, and configuration checks when MCP search is too token-heavy or unavailable.
---
...
Use this skill when a task needs current web information, source collection, URL content extraction, or Tavily site mapping through the local `grok-search` CLI instead of MCP tools.
...
```bash
./dist/grok-search search "query" --format json
```

- Fetch one URL as Markdown:

```bash
./dist/grok-search fetch "https://example.com/page"
```

- Map a documentation site:

```bash
./dist/grok-search map "https://docs.example.com" --instructions "only API docs" --format json
```

- Check configuration:

```bash
./dist/grok-search config check
```

If `./dist/grok-search` is missing and the workspace is this repository, build it:
...
If the binary is installed on `PATH`, use `grok-search` instead of `./dist/grok-search`.
...
6. Do not use MCP search tools for the same query unless the CLI fails or the user explicitly asks for MCP.
